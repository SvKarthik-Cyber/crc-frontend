...
const memberNavigation = [
  ...
  { label: 'My Incidents', to: 'incidents', end: true, icon: FolderKanban },
  { label: 'Report Incident', to: 'incidents/new', icon: FilePlus2 },
  { label: 'Advisories', to: 'advisories', icon: BookOpenText },
  { label: 'Notifications', to: 'notifications', icon: Bell },
  { label: 'Messages from CRC', to: 'messages', icon: MessagesSquare },
]
  ...
  return (
    <div className="flex min-h-svh min-w-0 flex-col bg-slate-50 text-slate-900">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-6 px-4 py-5 sm:px-6">
          <div className="grid min-w-0 gap-0.5">
            ...
            <span className="text-sm text-slate-600">Member Portal</span>
          </div>
          <Link
      ...
      <footer className="border-t border-slate-200 bg-white px-4 py-5 text-center text-sm text-slate-500">
        <p>Cyber Resilient Centre Member Portal</p>
      </footer>
    </div>
  )
}
